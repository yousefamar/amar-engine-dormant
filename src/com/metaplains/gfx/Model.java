...
public class Model {
	...
	public static void loadModels() {
		...
		CUBE = loadModel("cube").buildVBO();
		SPHERE = loadModel("icosphere").buildVBO();
		TREE = loadModel("tree").invertV().buildVBO();
		HOUSE = loadModel("house").invertV().buildVBO();
		MOHAWK = loadModel("mohawk").invertV().buildVBO();
		HEAD = loadModel("head").invertV().buildVBO();
		TORSO = loadModel("torso").invertV().buildVBO();
		QUAD = loadModel("quadcopter").invertV().buildVBO();
	}
	...
	private Model buildVBO() {
		this.id = Model.buildVBO(this);
		return this;
	}
	...
	private static int buildVBO(Model model) {
		int vboID = VBOHelper.createVBOID();
		int size = 0;
		for (Mesh mesh : model.meshes) {
			model.triCount += mesh.faces.size();
		...
		for (Mesh mesh : model.meshes) {
			...
			for (TexturedPolygon face : mesh.faces) {
				for (int i=0; i<face.vertexIDs.size(); i++) {
					buffer.put(model.vertices.get(face.vertexIDs.get(i)-1).x).put(model.vertices.get(face.vertexIDs.get(i)-1).y).put(model.vertices.get(face.vertexIDs.get(i)-1).z);
					if (!face.normalIDs.isEmpty())
						buffer.put(model.normals.get(face.normalIDs.get(i)-1).x).put(model.normals.get(face.normalIDs.get(i)-1).y).put(model.normals.get(face.normalIDs.get(i)-1).z);	
					if (!face.texCoordIDs.isEmpty())
						buffer.put(model.texCoords.get(face.texCoordIDs.get(i)-1).x).put(model.texCoords.get(face.texCoordIDs.get(i)-1).y);
				}
			}
		}
		buffer.flip();
		VBOHelper.bufferData(vboID, buffer, VBOHelper.STATIC_DRAW);
		return vboID;
	}
